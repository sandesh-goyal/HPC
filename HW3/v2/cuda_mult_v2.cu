...
#define N 10000								// Square Matrix dimension
...
{
	__shared__ float a_sh[BLOCK_DIM][BLOCK_DIM];
	__shared__ float b_sh[BLOCK_DIM][BLOCK_DIM];
	
	int a_loc = (((BLOCK_DIM * blockIdx.y) + threadIdx.y) * N) + threadIdx.x;
	int b_loc = (BLOCK_DIM * blockIdx.x) + (threadIdx.y * N) + threadIdx.x;
	...
	for(int j=0; j<t; j++)
	{
		a_sh[threadIdx.y][threadIdx.x] = a[a_loc];
		//b_sh[threadIdx.y][threadIdx.x] = b[b_loc];
		b_sh[threadIdx.x][threadIdx.y] = b[b_loc];
		__syncthreads();
		
		for(int i=0; i<BLOCK_DIM; i++)
		{
			//sum += a_sh[threadIdx.y][i] * b_sh[i][threadIdx.x];
			sum += a_sh[threadIdx.y][i] * b_sh[threadIdx.x][i];
		}
		__syncthreads();
	...
	c[(((BLOCK_DIM * blockIdx.y) + threadIdx.y) * N) + (BLOCK_DIM * blockIdx.x) + threadIdx.x] = sum;
}
//-----------------------------------------------------------------Main function
int main()
{	
